...
class CreateProductoCatalogosTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('producto_catalogos', function (Blueprint $table) {
            ...
            $table->decimal('existencias',20,6)->nullable();
            $table->decimal('disponible',20,6)->nullable();
            $table->decimal('asignado',20,6)->nullable();
            $table->decimal('precio_minimo',20,6)->nullable();
            $table->char('um',2);
            $table->foreign('um')->references('id')->on('unidad_medida_catalogos');
            $table->decimal('peso_neto',10,6)->nullable();
            $table->decimal('peso_bruto',10,6)->nullable();
            $table->decimal('largo',10,6)->nullable();
            $table->decimal('ancho',10,6)->nullable();
            $table->decimal('altura',10,6)->nullable();
            $table->decimal('costo_promedio',10,6)->nullable();
            $table->decimal('costo_ultima_produccion',10,6)->nullable();
            $table->decimal('total_costo_inventario',20,6)->nullable();
            $table->dateTime('fecha_ultimo_costo_promedio')->nullable();
            $table->dateTime('fecha_ultima_produccion')->nullable();
            $table->char('cuenta_inventario_id',8)->nullable();
            $table->foreign('cuenta_inventario_id')->references('id')->on('contabilidad_plan_cuentas');
