...
@section('title', 'Catálogo Permisos Autotransportes')
            ...
            <div class="col-12 table-responsive" >
                <div class="card">
                    <div class="card-header">
                        ...
                        <h3 class="titulo-tabla">Catálogo Permisos Autotransportes.</h3>
                    </div>
                    <div class="card-body">
            ...
            <div class="modal-content bg-danger">
                ...
                <div class="modal-body">
                    ...
                    <form action="javascript:void(0)" id="modal-form" name="modal-form"  method="post">
                        ...
                        <div class="form-group">
                            <label for="inputTipoPermiso">Seleccione El Tipo De Permiso</label>
                            <select class="custom-select" id="sat_carta_porte_tipo_permiso_catalogos_id">
                                @foreach($satPermisos as $satPermiso)
                                    <option value={{$satPermiso->id}}>{{$satPermiso->nombre}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
...
@section('js')
    <script>
        ...
        $(document).ready(function() {
            var titulo = 'Catálogo Tipo Permiso Vehicular Carta Porte SAT.';
            ...
            var table = $('#data-table').DataTable( {
                "ajax": "{{route('datatable.PermisoAutotransporteController')}}",
                "columns":[
                    {data : 'DT_RowIndex'},
                    {data : 'id'},
                    {data : 'permiso_nombre'},
            ...
            $('body').on('click', 'button.editButton', function () {
                var id = $(this).data('id');
                $.get("{{ route('permisoAutotrasnporte.index') }}" +'/' +id.trim()+ '/edit', function (data) {
                    $('#modal-container').modal('show');
                    $("#btn-save"). attr("id", "btn-edit");
                    $('#title-form').html("Editando ID: "+id+ " "+titulo);
                    $('#id').val(data.id);
                    $('#sat_carta_porte_tipo_permiso_catalogos_id').val(data.sat_carta_porte_tipo_permiso_catalogos_id);
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            url: "{{ url('permisoAutotrasnporte')}}" + '/' + id,
            ...
            $('body').on('click', '#btn-edit', function () {
                var id = $("#id").val();
                var sat_carta_porte_tipo_permiso_catalogos_id = $("#sat_carta_porte_tipo_permiso_catalogos_id").val();
                ...
                if(id.trim() == '' )
                {
                    ...
                    $.ajax({
                        type:"POST",
                        url: "{{route('permisoAutotrasnporte.index')}}",
                        data: {
                            id:id,
                            sat_carta_porte_tipo_permiso_catalogos_id:sat_carta_porte_tipo_permiso_catalogos_id,
                            vigencia:vigencia,
                            asignado:asignado,
                            activo:activo,
                        },
